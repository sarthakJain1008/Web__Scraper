// scrape-parallel-details.js
// Parallel batch detail scraper for all listings
// Supports 9 API keys running in parallel
// 5 URLs per batch, ~32K listings per API key
...
const MAX_RETRIES = 3;
const LISTINGS_PER_API = 29071; // ~29K listings per API key (290708 / 10)
...
async function parallelBatchScrape() {
  ...
  try {
    // Calculate range for this API key
    const totalListings = await collection.countDocuments({});
    const startIndex = apiKeyIndex * LISTINGS_PER_API;
    const endIndex = Math.min(startIndex + LISTINGS_PER_API, totalListings);
    
    console.log('📊 Assignment for this API key:');
    console.log(`  Total listings: ${totalListings.toLocaleString()}`);
    console.log(`  Start index: ${startIndex.toLocaleString()}`);
    console.log(`  End index: ${endIndex.toLocaleString()}`);
    console.log(`  Assigned: ${(endIndex - startIndex).toLocaleString()} listings`);
    console.log('');

    // Get assigned listings (not yet processed by this API)
    const query = {
      $or: [
        { detailsScrapedByApi: { $exists: false } }, // Not processed yet
        { detailsScrapedByApi: apiKeyIndex } // Processed by this API (resume)
      ]
    };

    const allListings = await collection
      .find(query, { projection: { url: 1, listing_id: 1, detailsScrapedByApi: 1 } })
      .skip(startIndex)
      .limit(LISTINGS_PER_API)
      .toArray();

    console.log(`✅ Found ${allListings.length.toLocaleString()} listings to process`);
    console.log('');

    // Check for already processed
    const alreadyProcessed = allListings.filter(l => l.detailsScrapedByApi === apiKeyIndex).length;
    if (alreadyProcessed > 0) {
      console.log(`🔄 RESUME MODE: ${alreadyProcessed} listings already processed by this API`);
      console.log(`   Will process remaining ${allListings.length - alreadyProcessed} listings`);
      console.log('');
    }

    // Filter out already processed
    const toProcess = allListings.filter(l => l.detailsScrapedByApi !== apiKeyIndex);

    if (toProcess.length === 0) {
      console.log('✅ All assigned listings already processed!');
      return;
    }
          ...
          if (items.length > 0) {
            for (const item of items) {
              ...
              const updateData = {
                ...
                detailsScrapedApiOffset: startIndex + (batchIndex * BATCH_SIZE)
              };
